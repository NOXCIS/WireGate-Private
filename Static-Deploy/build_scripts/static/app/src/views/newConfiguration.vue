...
export default {
  ...
  // Update existing updateIPTablesScripts method
  updateIPTablesScripts() {
      ...
      if (previousScripts.PostUp !== this.newConfiguration.PostUp ||
  ...
  reader.readAsText(file);
    ...
    prepareApiData() {
      ...
      // Generate appropriate IPTables scripts based on toggle state
  const generatedScripts = this.iptablesEnabled ? 
    {
    ...
    } : 
    {
      ...
      const apiData = {
        ...
        PublicKey,
        PresharedKey,
        PreDown: generatedScripts.PreDown,  // Always include generated PreDown
        PostUp: generatedScripts.PostUp,
        PostDown,
		Protocol,
      };
  ...
  'newConfiguration.PreDown'(newVal, oldVal) {
    ...
    //good
		// Add new watchers for IPTables-related fields
    // Update ConfigurationName watcher
    'newConfiguration.ConfigurationName'(newVal) {
      // Existing validation logic
      let ele = document.querySelector("#ConfigurationName");
      ele.classList.remove("is-invalid", "is-valid");
      
      const isValid = !/^[a-zA-Z0-9_=+.-]{1,15}$/.test(newVal) || 
                     newVal.length === 0 || 
      ...
      if (isValid) {
        ...
        ele.classList.add("is-valid");
        // Always update IPTables scripts when name is valid
        this.updateIPTablesScripts();
      }
    },

    // Update Address watcher
    'newConfiguration.Address'(newVal) {
      ...
      try {
        ...
        // Always update IPTables scripts when address is valid
        this.updateIPTablesScripts();
      } catch (e) {
    ...
    // Watch for IPTables toggle
    iptablesEnabled(newVal) {
      this.updateIPTablesScripts();
    },

    // Add deep watcher for the entire newConfiguration object
    newConfiguration: {
      deep: true,
      handler(newVal, oldVal) {
        ...
        if (hasRelevantChanges && this.iptablesEnabled) {
          this.updateIPTablesScripts();
        }
      }
    },
          ...
          <div v-if="selectedField" class="card rounded-3 shadow">
            ...
            <div class="card-body p-0">
              <textarea
                ...
                class="form-control script-box-active border-0 font-monospace resizable-textarea"
